...
import { GetDataByID } from '../../../api/ManagementUnitAPI';

const RestoreViewMU = ({ setOpen, open, rowRestore }) => {
  const style = {
    position: 'absolute',
    top: '40%',
    left: '50%',
  ...
  const getManagementUnitData = async (rowRestore) => {
    await GetDataByID(rowRestore.id)
  ...
  useEffect(() => {
    if (open === true) {
      getManagementUnitData(rowRestore);
    }
        ...
        <Card sx={style}>
          <Box sx={{ mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Typography id="modal-modal-title" variant="h5">
              Show Details of Management Unit
            </Typography>
...
export default RestoreViewMU;

RestoreViewMU.propTypes = {
