...
import { GetCTAssignmentHistory, getCtHistoryIdData } from '../../api/ContactTypeApi';
...
function AssignmentHistory() {
  ...
  const handleContactClick = () => {
    navigate('/dashboard/account-setup/contact-type');
  };

  const getHistoryData = useCallback(async () => {
    await GetCTAssignmentHistory(type, id)
      .then((response) => setHistoryData(response.data))
      .catch((err) => {
        console.log(err);
      });

    await getCtHistoryIdData(id)
    ...
    <div>
      <Card>
        <Box sx={{ m: 2, display: 'flex', alignItems: 'center', flexDirection: 'row' }}>
          <Typography style={{ cursor: 'pointer' }} id="modal-modal-title" variant="h5" onClick={handleContactClick}>
            Contact Type
          </Typography>
        ...
        <Typography sx={{ m: 2, display: 'flex', alignItems: 'center' }} id="modal-modal-title" variant="h6">
          {storeData?.ct_name}/{storeData.id}
        </Typography>
