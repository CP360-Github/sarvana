...
const users = [
  {
    ...
    role: 'admin',
    isPublic: true,
    userName: 'scheduly_admin'
  }
];

// ----------------------------------------------------------------------

mock.onPost('https://scheduly.o2ohrms.com/auth/login/').reply(async (config) => {
  try {
    ...
    if (!user) {
      return [400, { message: 'There is no user corresponding to the username.' }];
    }
